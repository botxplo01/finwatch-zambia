...
/**
 * FinWatch Zambia - Regulator Documentation Content Layout
 */
...
import { RegulatorDocsSidebar } from "./RegulatorDocsSidebar";
import { RegulatorDocsSearchModal } from "./RegulatorDocsSearchModal";
import { cn } from "@/lib/utils";

interface RegulatorDocsContentLayoutProps {
  children: ReactNode;
  title: string;
...
export function RegulatorDocsContentLayout({
  children,
  title,
  previousSection,
  nextSection,
}: RegulatorDocsContentLayoutProps) {
      ...
      <div className="flex flex-col gap-12 lg:flex-row">
        ...
        <aside className="hidden w-64 flex-shrink-0 lg:block">
          <div className="sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-border">
            ...
            <RegulatorDocsSidebar />
          </div>
        </aside>

        {/* Search Modal */}
        <RegulatorDocsSearchModal
          isOpen={isSearchOpen}
              ...
              <div className="overflow-y-auto flex-1 pb-10">
                ...
                <div className="mb-6 relative group">
                  ...
                  <input
                    ...
                    className="w-full h-12 pl-11 pr-4 py-2.5 text-sm border border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50 rounded-xl focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100 dark:focus:ring-emerald-900/40 text-zinc-900 dark:text-zinc-100 transition-all shadow-sm"
                  />
                </div>
          ...
          <div className="mt-16 flex flex-col gap-4 border-t border-border pt-8 sm:flex-row sm:items-center sm:justify-between">
            {previousSection ? (
              <Link
                ...
                className="group flex flex-col gap-1 rounded-lg border border-border p-4 transition-all hover:border-emerald-600/50 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/5 sm:w-1/2"
              >
                <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  <ArrowLeft className="h-3 w-3" /> Previous
                </span>
                <span className="text-sm font-semibold group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
            ...
            {nextSection ? (
              <Link
                ...
                className="group flex flex-col items-end gap-1 rounded-lg border border-border p-4 transition-all hover:border-emerald-600/50 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/5 sm:w-1/2"
              >
                <span className="flex items-center gap-1 text-xs text-muted-foreground">
