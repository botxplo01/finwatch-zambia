...
export default function RegulatorReportsPage() {
  ...
  // Export overview as JSON (CSV/PDF export would require a backend endpoint)
  async function handleExport() {
    setExporting(true);
    setExportMsg("");
    try {
      const [ovRes, secRes, trendRes] = await Promise.all([
        api.get("/api/regulator/overview", { headers: getRegAuthHeader() }),
        api.get("/api/regulator/sectors", { headers: getRegAuthHeader() }),
        api.get("/api/regulator/trends", { headers: getRegAuthHeader() }),
      ]);

      const exportData = {
        exported_at: new Date().toISOString(),
        note: "Anonymised aggregate data only — FinWatch Zambia Regulator Portal",
        overview: ovRes.data,
        sector_distress: secRes.data,
        monthly_trends: trendRes.data,
      ...
      link.href = url;
      link.download = `finwatch-regulator-export-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  return (
    <div className="p-6 pb-24 max-w-7xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        {/* Export — full regulators only */}
        {isFullReg && (
          <button
            onClick={handleExport}
            disabled={exporting}
            className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white rounded-xl transition-all hover:opacity-90 active:scale-95 disabled:opacity-60 shadow-sm flex-shrink-0"
      ...
      <div className="flex items-start gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3">
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-blue-600/80 dark:text-blue-400/70">
            ...
            IDs, or personally identifiable information is included. Exports are
            permissible under the FinWatch data governance policy.
          </p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl px-5 py-4 space-y-2">
            ...
            <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed">
              ...
              datasets, and <strong>Logistic Regression</strong> for its
              interpretability and baseline comparison. Both models are trained
              on the UCI Polish Companies Bankruptcy dataset
              (doi:10.24432/C5V61K) and contextualised against World Bank Zambia
              Enterprise Survey data (2019–2020). SHAP attributions provide
              ratio-level explainability per prediction.
            </p>
          </div>
        </>
      )}
