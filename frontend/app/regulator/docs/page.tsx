      ...
      <section className="w-full relative py-20 px-4 border-b border-border bg-white dark:bg-[#0a0a0a] z-30">
        ...
        <div className="container mx-auto max-w-4xl text-center relative z-10">
          <h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl text-zinc-900 dark:text-zinc-100">
            Institutional Oversight & <span className="text-emerald-600">Analytics Guide</span>
          </h1>
          <p className="mt-4 text-base text-muted-foreground max-w-xl mx-auto leading-relaxed">
            Technical documentation for regulators and policy analysts monitoring systemic financial health.
          </p>
          ...
          <div ref={searchRef} className="relative mt-12 max-w-xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
            <div className="relative group">
              ...
              <Search className="absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-zinc-500 group-focus-within:text-emerald-600 transition-colors z-30 pointer-events-none" size={16} />
            </div>
              ...
              <div className="absolute top-[calc(100%+0.75rem)] left-0 right-0 z-[100] rounded-[1.5rem] border border-zinc-100 dark:border-zinc-800 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-top-2 duration-300 overflow-hidden flex flex-col text-left">
                {results.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-muted-foreground">
                    ...
                    <p className="text-sm font-medium">No institutional matches found.</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
          {CARDS.map((card, i) => {
            ...
            const colorClasses: Record<string, string> = {
              ...
              slate: "bg-slate-50 text-slate-600 dark:bg-slate-900/20 dark:text-slate-400 group-hover:bg-slate-600",
            };
      ...
      <section className="container mx-auto pb-24 px-4 sm:px-6">
        <div className="rounded-3xl bg-zinc-950 p-8 sm:p-12 dark:bg-emerald-900/10 border border-white/5 overflow-hidden relative shadow-2xl">
          <div className="relative z-10 flex flex-col items-center text-center max-w-2xl mx-auto">
            <h2 className="text-2xl font-bold text-white mb-4">Institutional Support</h2>
            <p className="text-zinc-400 mb-8">Access dedicated guidance on systemic risk modeling, anonymization protocols, and regulatory reporting features.</p>
            <div className="flex items-center gap-4">
              <Link href="/regulator/docs/faq" className="rounded-full bg-white px-8 py-3 text-sm font-bold text-zinc-950 transition-transform hover:scale-105 active:scale-95">Read FAQ</Link>
              <button
                onClick={() => {
