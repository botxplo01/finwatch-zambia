...
def _collect_all_data(db: Session) -> dict:
    """Fetch comprehensive aggregate data from the database."""
...
def generate_regulator_pdf(db: Session, user_time: str | None = None) -> tuple[bytes, str]:
    """Generate a detailed institutional aggregate PDF report with modern styling."""
    data = _collect_all_data(db)
    filename = f"finwatch_regulator_aggregate_{datetime.now(timezone.utc).strftime('%Y%m%d')}.pdf"
    
    styles = _build_styles()
    story = []
    
    # 1. Title & Institutional Metadata
    story.append(Paragraph("System-Wide Aggregate Performance Report", styles["title"]))
...
def generate_regulator_csv(db: Session) -> tuple[bytes, str]:
    ...
    data = _collect_all_data(db)
    filename = f"finwatch_regulator_aggregate_{datetime.now(timezone.utc).strftime('%Y%m%d')}.csv"
    output = io.StringIO(); writer = csv.writer(output)
    writer.writerow(["FINWATCH REGULATOR AGGREGATE REPORT"])
...
def generate_regulator_json(db: Session) -> tuple[bytes, str]:
    ...
    data = _collect_all_data(db)
    filename = f"finwatch_regulator_aggregate_{datetime.now(timezone.utc).strftime('%Y%m%d')}.json"
    return json.dumps(data, indent=2).encode("utf-8"), filename

def generate_regulator_zip(db: Session, user_time: str | None = None) -> tuple[bytes, str]:
    """Bundle all detailed aggregate formats into a ZIP."""
    pdf_bytes, pdf_name = generate_regulator_pdf(db, user_time=user_time)
    ...
    return zip_buf.getvalue(), f"finwatch_regulator_bundle_{datetime.now(timezone.utc).strftime('%Y%m%d')}.zip"
